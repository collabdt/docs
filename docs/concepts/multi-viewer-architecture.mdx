...
### Point clouds, inside the BIM viewer
...
Reality capture datasets that would be impractical to load in a GIS tool render inside the BIM viewer rather than in a viewer of their own. Tile-based streaming and LOD keep billions of points interactive, and because the cloud is an object in the BIM scene it shares that scene's camera, clipping planes and measurement tools — which is what makes as-built against design a single comparison rather than two windows.

Uploads are converted server-side into a Potree octree; see the [point cloud guide](../guides/point-cloud-viewer.md).
